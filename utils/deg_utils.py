...
def mri_mask_to(tensor, mask_root='data/datasets/gt_keep_masks/genhalf', mask_id=-1, n=1):
    ...
    #mask = F.interpolate(mask, size=tensor.shape[2:], mode='nearest')
    masked_tensor = mask * tensor
    return {"white":masked_tensor + (1. - mask),"black":masked_tensor,"mask":mask}
# def multi_mri_mask_to(tensor, mask_root='data/datasets/gt_keep_masks/genhalf', mask_id=-1, n=1):
#     from scipy.io import loadmat
#     # mask_name = os.listdir(mask_root)
#     batch = tensor.shape[0]
#     if mask_id < 0:
#         mask_id = 0
#         mask_temp = loadmat(mask_root)['mask']
#         mask_temp = np.repeat(mask_temp[None, ...], tensor.shape[1], 0)
#         mask = np.repeat(mask_temp[None,...], batch, 0)
#         # masks = []
#         # for i in range(batch):
#         #     mask_temp = loadmat(os.path.join(mask_root, mask_name[mask_id[i]]))['mask']
#         #     mask_temp = np.stack([mask_temp, mask_temp], axis= 0)#2,256,256
...
#     #mask = F.interpolate(mask, size=tensor.shape[2:], mode='nearest')
#     masked_tensor = mask * tensor
#     return {"white":masked_tensor + (1. - mask),"black":masked_tensor}
    # return masked_tensor
######## super-resolution ###########

def multi_mri_mask_to(tensor, mask_root='data/datasets/gt_keep_masks/genhalf', mask_id=-1, n=10):
    from scipy.io import loadmat
    mask_name = os.listdir(mask_root)
    batch = tensor.shape[0]
    if mask_id < 0:
        mask_id = np.random.randint(n)
        mask_temp = loadmat(os.path.join(mask_root, mask_name[mask_id]))['mask']
        mask_temp = np.repeat(mask_temp[None, ...], tensor.shape[1], 0)
        mask = np.repeat(mask_temp[None,...], batch, 0)
        # masks = []
        # for i in range(batch):
