...
def write_Data_p(model_num,psnr,ssim,img_name):
    filedir="result_psnr_main.txt"
    with open(osp.join('./Res/',img_name+filedir),"w+") as f:#a+
        f.writelines(str(model_num)+' '+'['+str(round(psnr, 2))+' '+str(round(ssim, 4))+']')
        f.write('\n')
def write_Data_s(model_num,psnr,ssim,img_name):
    filedir="result_ssim_main.txt"
    with open(osp.join('./Res/',img_name+filedir),"w+") as f:#a+
        f.writelines(str(model_num)+' '+'['+str(round(psnr, 2))+' '+str(round(ssim, 4))+']')
        f.write('\n')        
def write_Data2(psnr,ssim,img_name):
    filedir="PC.txt"
    with open(osp.join('./Res/',img_name+filedir),"a+") as f:#a+
...
class SDE(abc.ABC):
    ...
    def reverse_sde_step(self, x, score, t,i):
        return x - self.sde_reverse_drift(x, score, t,i) - self.dispersion(x, t)
...
class IRSDE(SDE):
    ...
    def sde_reverse_drift(self, x, score, t,i):
        return (self.thetas[t] * (self.mu[i,:,:,:][None,...] - x) - self.sigmas[t]**2 * score) * self.dt
    ...
    def score_fn(self, x, t,i):
        # need to pre-set mu and score_model
        noise = self.model(x, self.mu[i,:,:,:][None,...], t)
    ...
    def reverse_sde(self, xt, T=-1, save_states=False, save_dir='sde_state',ori = None, mask = None, weight = None,
                    opt=None,model=None):
        ...
        x_input = k_w_8_ori
        xt = torch.tensor(x_input, dtype=torch.float32).cuda()  # 12,2,256,256

        x_mean = xt.clone()

        for t in tqdm(reversed(range(1, T + 1))):

            for batch in range(coil):

                score = self.score_fn(x_mean[batch,:,:,:][None,...], t,batch)
                x_mean[batch,:,:,:] = self.reverse_sde_step(x_mean[batch,:,:,:][None,...], score, t,batch).squeeze()

            x_mean = x_mean.cpu().numpy()
            x_mean = np.array(x_mean,dtype=np.float32)
            output_k_complex = np.zeros((coil,256, 256), dtype=np.complex64)
            output_k_4_complex = np.zeros((coil,int(x_mean.shape[1] / 2), 256, 256), dtype=np.complex64)
            for i in range(coil):
                for j in range(output_k_4_complex.shape[1]):
                    output_k_4_complex[i,j, :, :] = x_mean[i, j,:, :] + 1j * x_mean[i,j + 4, :, :]
                    output_k_complex[i,:,:] = np.multiply(output_k_4_complex[i,j ,:, :], mask_muti[j,:,:]) + output_k_complex[i,:,:]
                    # output_k_complex[i, :, :] = output_k_4_complex[i, j, :, :]+ output_k_complex[i,:,:]

                output_k_complex[i,:,:] = np.multiply(output_k_complex[i,:,:], 1 / mask_sum)

            A_new_complex = output_k_complex
            ...
            #SVD
            k = math.floor(wnthresh*ksize[0]*ksize[1])
            uu,ss,vv = L_SVD(A,k)
            ...
            # k_complex3 = k_complex2/ np.max(np.abs(k_complex2))
            #
            # if t in [100,200,300,400,500,600]:
            #     for i in range(coil):
            #         write_images(np.abs(k_complex3[i,:,:]), osp.join('./Res/', img_name + f'{t}k_space_'+f'{coil}'+ '.png'))
            #
            #     k_sos = np.fft.fft2(rec_Image_sos)
            #     write_images(np.abs(k_sos), osp.join('./Res/',  f'{t}k_space_sos' + '.png'))
            ...
            for i in range(coil): 
                ...
                # k_w[i,:,:] = k2wgt(k_complex[i,:,:]*(mask[i,:,:]),weight[i,:,:])
            ...
            psnr = compare_psnr(255*np.abs(rec_Image_sos),255*np.abs(ori_data_sos),data_range=255)
            ssim = compare_ssim(np.abs(rec_Image_sos),np.abs(ori_data_sos),data_range=1)

            print(' PSNR:', psnr,' SSIM:', ssim) 
            write_Data2(psnr,ssim,img_name)

            if max_ssim <= ssim:
                max_ssim = ssim
                max_ssim_psnr = psnr
                write_Data_s('checkpoint',psnr,ssim,img_name)
                write_images(np.abs(rec_Image_sos),osp.join('./Res/',img_name+'Rec_ssim_main'+'.png'))
                savemat(osp.join('./Res/',img_name+'rec_s.mat'),{'Img':rec_Image})
            if max_psnr <= psnr:
                max_psnr = psnr
                max_psnr_ssim = ssim
                write_Data_p('checkpoint',psnr,ssim,img_name)
                write_images(np.abs(rec_Image_sos),osp.join('./Res/',img_name+'Rec_psnr_main'+'.png'))
                savemat(osp.join('./Res/',img_name+'rec_p.mat'),{'Img':rec_Image})
        ...
        return x_mean,{"psnr":max_psnr,"ssim":max_psnr_ssim,"zf_psnr":psnr_zf,"zf_ssim":ssim_zf}
