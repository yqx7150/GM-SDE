...
class GTDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train":
            ...
            img_GT = img_GT[:,:,0]+1j*img_GT[:,:,1]
        elif self.opt["phase"] == "save":
            # img_GT = loadmat(GT_path)['Img2']
            # img_GT = img_GT[:,:,0]+1j*img_GT[:,:,1]
            ...
            img_GT = loadmat(GT_path)['Img2']
            img_GT = img_GT[:, :, 0] + 1j * img_GT[:, :, 1]
        img_GT_nor = img_GT/np.max(np.abs(img_GT)) 
        img_GT_k = np.fft.fftshift(np.fft.fft2(img_GT_nor))

        img_GT_K_w = self.k2wgt(img_GT_k, weight)

        w_k = np.stack((np.real(img_GT_K_w),np.imag(img_GT_K_w)), 2)#256,256,2
        ...
        return {"GT": w_k, "GT_path": GT_path, "GT_k_no_w": img_GT_k,"ori_img":img_GT}
        #return img_GT
