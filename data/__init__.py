...
def create_dataloader(dataset, dataset_opt, opt=None, sampler=None):
    ...
    #print(phase)
    #print(dataset_opt["phase"])
    if phase == "train":
        #print(opt['dist'])#False
        if opt["dist"]:
            ...
            num_workers = dataset_opt["n_workers"] * len(opt["gpu_ids"])#4
            batch_size = dataset_opt["batch_size"]#4
...
def create_dataset(dataset_opt):
    ...
    if mode == "LQ":  # Predictor
        ...
        dataset = D(dataset_opt)
    elif mode == "pi":  # Corrector
        from data.GT_datasetp import GTDataset as D
