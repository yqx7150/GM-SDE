...
class DenoisingModel(BaseModel):
    ...
    def test(self, sde=None, save_states=False,
             ori = None, mask = None, weight = None,opt=None,model=None,mode = 'test',img_name=None,sample_k=None):
        sde.set_mu(self.condition)

        self.model.eval()
        with torch.no_grad():
            # if mode == 'test':
            #     self.output,data = sde.reverse_sde(self.state, save_states=save_states,
            #                                     ori = ori, mask = mask, weight = weight,opt=opt,model=model,img_name=img_name)
            if mode == 'test':
                self.output,data = sde.reverse_sde(self.state, save_states=save_states,
                                                ori = ori, mask = mask, weight = weight,opt=opt,model=model)
            elif mode == 'test_save':
                self.output,data_save = sde.reverse_sde(self.state, save_states=save_states,
                                                ori = ori, sample_k = sample_k, weight = weight,opt=opt,model=model,img_name=img_name,mask = mask)
            else:
